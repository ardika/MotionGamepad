...
public class GamepadInterface {
	...
	public GamepadInterface() {
		...
		//_PacketPattern = Pattern.compile("X(.{3})Y(.{3})M(.{3})S(\\d{1})(\\d{1})(\\d{1})E");
	...
	private GamepadSerialCommunication GetSerial() {
		return _SerialComm;
	}
        ...
        public void run ()
        {
            while (this._Interface.IsGamepadConnected())
            {
            	if (_Interface.GetSerial().BytesToRead() > 0) {
            		...
            		System.out.println(_Interface.GetButtonAState());
            		try {
                		Thread.sleep(50);
                	} catch (Exception Ex) {};
            	}
            	else {
            		_Interface.SendToogleTransmissionCommand();
            	}
            }       
        }
    }
